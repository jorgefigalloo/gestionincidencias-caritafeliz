...
<div class="container mx-auto px-4 py-8 max-w-7xl">
    ...
    <div class="flex flex-col lg:flex-row justify-between items-start lg:items-center mb-8 gap-4">
        <div>
            ...
            <p class="text-sm text-gray-500 mt-1">Administra las ubicaciones físicas de la organización</p>
        </div>
        ...
        <div class="flex flex-col sm:flex-row gap-3 w-full lg:w-auto">
            ...
            <button id="add-sede-btn" class="bg-teal-600 hover:bg-teal-700 text-white font-medium py-2.5 px-5 rounded-xl shadow-lg shadow-teal-500/30 transition-all duration-300 flex items-center justify-center whitespace-nowrap transform hover:-translate-y-0.5">
                <i class="fas fa-building mr-2"></i> Nueva Sede
            </button>
        </div>
    </div>
    ...
    <div id="loading" class="hidden py-12 flex flex-col items-center justify-center text-teal-600">
        ...
        <p class="text-sm font-medium text-gray-500">Cargando sedes...</p>
    </div>

    <!-- Table -->
    <div id="sedes-table-container" class="bg-white shadow-sm border border-gray-100 rounded-2xl overflow-hidden">
        <div class="overflow-x-auto">
            <table class="w-full text-left border-collapse">
                ...
                <tbody id="sedes-table-body" class="divide-y divide-gray-100">
    ...
    <div id="no-data-message" class="hidden bg-white shadow-sm border border-gray-100 rounded-2xl p-12 text-center">
        ...
        <h3 class="text-lg font-bold text-gray-900 mb-1">No hay sedes</h3>
        <p class="text-gray-500 mb-6">Comienza registrando una nueva sede en el sistema.</p>
        <button onclick="document.getElementById('add-sede-btn').click()" class="text-teal-600 font-medium hover:text-teal-700">
            Crear primera sede &rarr;
        </button>
    </div>
    ...
    <div id="no-search-results" class="hidden bg-white shadow-sm border border-gray-100 rounded-2xl p-12 text-center">
        ...
        <p class="text-gray-500 mb-6">No encontramos sedes que coincidan con tu búsqueda.</p>
...
<!-- Modal -->
<div id="sede-modal" class="fixed inset-0 z-50 hidden overflow-y-auto" aria-labelledby="modal-title" role="dialog" aria-modal="true">
    <div class="flex items-end justify-center min-h-screen pt-4 px-4 pb-20 text-center sm:block sm:p-0">
        ...
        <div class="inline-block align-bottom bg-white rounded-2xl text-left overflow-hidden shadow-xl transform transition-all sm:my-8 sm:align-middle sm:max-w-md sm:w-full">
            <div class="bg-white px-6 py-4 border-b border-gray-100 flex items-center justify-between">
                <h3 class="text-lg font-bold text-gray-900" id="modal-title">Nueva Sede</h3>
            ...
            <form id="sede-form" class="px-6 py-6">
                ...
                <div class="mb-6">
                    ...
                    <input type="text" id="sede-nombre" required maxlength="50"
                ...
                <div class="mb-6">
                    ...
                    <textarea id="sede-descripcion" rows="3" maxlength="255"
                        class="w-full px-4 py-3 bg-gray-50 border border-gray-200 rounded-xl text-sm focus:ring-2 focus:ring-teal-500 focus:border-teal-500 transition-all"
            ...
            <div class="bg-gray-50 px-6 py-4 flex flex-col-reverse sm:flex-row sm:justify-end gap-3 border-t border-gray-100">
                ...
                <button type="submit" id="save-btn" form="sede-form" class="w-full sm:w-auto px-6 py-2.5 bg-teal-600 text-white font-medium rounded-xl hover:bg-teal-700 shadow-lg shadow-teal-500/30 transition-all flex items-center justify-center">
                    <span id="save-btn-text">Guardar Sede</span>
...
<div id="delete-confirm-modal" class="fixed inset-0 z-50 hidden overflow-y-auto" aria-labelledby="modal-title" role="dialog" aria-modal="true">
    <div class="flex items-end justify-center min-h-screen pt-4 px-4 pb-20 text-center sm:block sm:p-0">
        ...
        <div class="inline-block align-bottom bg-white rounded-2xl text-left overflow-hidden shadow-xl transform transition-all sm:my-8 sm:align-middle sm:max-w-sm sm:w-full p-6 text-center">
            ...
            <h3 class="text-lg font-bold text-gray-900 mb-2">¿Eliminar sede?</h3>
            <p class="text-sm text-gray-500 mb-6">Esta acción no se puede deshacer. La sede será eliminada permanentemente.</p>
...
<script>
    ...
    // DOM Elements
    const dom = {
        tableBody: document.getElementById('sedes-table-body'),
        tableContainer: document.getElementById('sedes-table-container'),
        ...
        modal: {
            self: document.getElementById('sede-modal'),
            title: document.getElementById('modal-title'),
            form: document.getElementById('sede-form'),
            ...
            inputs: {
                id: document.getElementById('sede-id'),
                nombre: document.getElementById('sede-nombre'),
                descripcion: document.getElementById('sede-descripcion')
            }
        },
        deleteModal: {
    ...
    function filterSedes() {
        if (!currentSearchTerm.trim()) {
            ...
            filteredSedes = allSedes.filter(s => 
    ...
    function renderSedes(sedes) {
        ...
        if (sedes.length > 0) {
            ...
            sedes.forEach(s => {
                ...
                row.innerHTML = `
                    <td class="px-6 py-4 text-xs font-mono font-bold text-gray-400 group-hover:text-teal-600">#${s.id_sede}</td>
                    <td class="px-6 py-4">
                        <div class="text-sm font-bold text-gray-900">${escapeHtml(s.nombre)}</div>
                    </td>
                    <td class="px-6 py-4">
                        <div class="text-sm text-gray-500">${escapeHtml(s.descripcion || '-')}</div>
                    </td>
                    <td class="px-6 py-4 text-right">
                        <div class="flex items-center justify-end gap-2">
                            <button class="edit-btn p-2 text-teal-600 hover:bg-teal-50 rounded-lg transition-colors" 
                                ...
                                data-descripcion="${escapeHtml(s.descripcion || '')}"
                                title="Editar">
    ...
    // Modal Actions
    document.getElementById('add-sede-btn').addEventListener('click', () => {
        dom.modal.form.reset();
        dom.modal.inputs.id.value = '';
        dom.modal.title.textContent = 'Nueva Sede';
    ...
    dom.modal.form.addEventListener('submit', async (e) => {
        ...
        const formData = {
            nombre: dom.modal.inputs.nombre.value,
            descripcion: dom.modal.inputs.descripcion.value
        };

        if (id) formData.id_sede = parseInt(id);
        ...
        try {
            ...
            if (res.ok) {
                showToast(result.message || 'Sede guardada', 'success');
            ...
            dom.modal.saveText.textContent = 'Guardar Sede';
    ...
    dom.tableBody.addEventListener('click', (e) => {
        ...
        if (btn.classList.contains('edit-btn')) {
            ...
            dom.modal.inputs.descripcion.value = btn.dataset.descripcion;
            dom.modal.title.textContent = 'Editar Sede';
            dom.modal.self.classList.remove('hidden');
        }
        
        if (btn.classList.contains('delete-btn')) {
            sedeIdToDelete = btn.dataset.id;
    ...
    dom.deleteModal.confirm.addEventListener('click', async () => {
        ...
        try {
            const res = await fetch(API_URL, {
                method: 'DELETE',
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify({ id_sede: parseInt(sedeIdToDelete) })
            });
            const result = await res.json();
