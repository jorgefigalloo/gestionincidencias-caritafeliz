...
<div class="container mx-auto px-4 py-8 max-w-7xl">
    ...
    <div class="flex flex-col lg:flex-row justify-between items-start lg:items-center mb-8 gap-4">
        <div>
            <h1 class="text-2xl font-bold text-gray-900">Subtipos de Incidencia</h1>
            <p class="text-sm text-gray-500 mt-1">Gestiona las clasificaciones específicas para cada tipo de incidencia</p>
        </div>
        ...
        <div class="flex flex-col sm:flex-row gap-3 w-full lg:w-auto">
            ...
            <button id="add-btn" class="bg-teal-600 hover:bg-teal-700 text-white font-medium py-2.5 px-5 rounded-xl shadow-lg shadow-teal-500/30 transition-all duration-300 flex items-center justify-center whitespace-nowrap transform hover:-translate-y-0.5">
    ...
    <div class="bg-white rounded-2xl shadow-sm border border-gray-100 p-5 mb-6">
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
            <div class="relative">
                <label class="block text-xs font-medium text-gray-500 mb-1.5 ml-1">Tipo de Incidencia</label>
    ...
    <div id="loading" class="hidden py-12 flex flex-col items-center justify-center text-teal-600">
        ...
        <p class="text-sm font-medium text-gray-500">Cargando subtipos...</p>
    </div>

    <!-- Table -->
    <div id="table-container" class="bg-white shadow-sm border border-gray-100 rounded-2xl overflow-hidden">
        <div class="overflow-x-auto">
            <table class="w-full text-left border-collapse">
                <thead>
                    <tr class="bg-gray-50/50 border-b border-gray-100 text-xs uppercase tracking-wider text-gray-500 font-semibold">
                        <th class="px-6 py-4">ID</th>
                        <th class="px-6 py-4">Subtipo</th>
    ...
    <!-- Empty States -->
    <div id="no-data" class="hidden bg-white shadow-sm border border-gray-100 rounded-2xl p-12 text-center">
        <div class="w-16 h-16 bg-gray-50 rounded-full flex items-center justify-center mx-auto mb-4">
            <i class="fas fa-list-ul text-3xl text-gray-300"></i>
        </div>
        <h3 class="text-lg font-bold text-gray-900 mb-1">No hay subtipos</h3>
        <p class="text-gray-500 mb-6">Comienza registrando un nuevo subtipo en el sistema.</p>
        <button onclick="document.getElementById('add-btn').click()" class="text-teal-600 font-medium hover:text-teal-700">
            Crear primer subtipo &rarr;
        </button>
    </div>
</div>
    ...
    <div class="flex items-end justify-center min-h-screen pt-4 px-4 pb-20 text-center sm:block sm:p-0">
        ...
        <div class="inline-block align-bottom bg-white rounded-2xl text-left overflow-hidden shadow-xl transform transition-all sm:my-8 sm:align-middle sm:max-w-md sm:w-full">
            <div class="bg-white px-6 py-4 border-b border-gray-100 flex items-center justify-between">
                <h3 class="text-lg font-bold text-gray-900" id="modal-title">Nuevo Subtipo</h3>
            ...
            <form id="subtipo-form" class="px-6 py-6">
                ...
                <div class="mb-6">
                    ...
                    <select id="tipo-select" required class="w-full px-4 py-3 bg-gray-50 border border-gray-200 rounded-xl text-sm focus:ring-2 focus:ring-teal-500 focus:border-teal-500 transition-all">
                ...
                <div class="mb-6">
                    ...
                    <select id="estado-select" class="w-full px-4 py-3 bg-gray-50 border border-gray-200 rounded-xl text-sm focus:ring-2 focus:ring-teal-500 focus:border-teal-500 transition-all">
                        <option value="activo">Activo</option>
                        <option value="inactivo">Inactivo</option>
                    </select>
                </div>
            ...
            <div class="bg-gray-50 px-6 py-4 flex flex-col-reverse sm:flex-row sm:justify-end gap-3 border-t border-gray-100">
                <button type="button" id="cancel-btn" class="w-full sm:w-auto px-6 py-2.5 bg-white border border-gray-300 text-gray-700 font-medium rounded-xl hover:bg-gray-50 transition-all">
                    Cancelar
                </button>
                <button type="submit" form="subtipo-form" class="w-full sm:w-auto px-6 py-2.5 bg-teal-600 text-white font-medium rounded-xl hover:bg-teal-700 shadow-lg shadow-teal-500/30 transition-all flex items-center justify-center">
...
<div id="delete-modal" class="fixed inset-0 z-50 hidden overflow-y-auto" aria-labelledby="modal-title" role="dialog" aria-modal="true">
    <div class="flex items-end justify-center min-h-screen pt-4 px-4 pb-20 text-center sm:block sm:p-0">
        ...
        <div class="inline-block align-bottom bg-white rounded-2xl text-left overflow-hidden shadow-xl transform transition-all sm:my-8 sm:align-middle sm:max-w-sm sm:w-full p-6 text-center">
            ...
            <h3 class="text-lg font-bold text-gray-900 mb-2">¿Eliminar subtipo?</h3>
            <p class="text-sm text-gray-500 mb-6">Esta acción no se puede deshacer. El subtipo será eliminado permanentemente.</p>
...
<script>
    ...
    let subtiposData = [];
    let tiposData = [];
    let filteredSubtipos = [];
    let currentFilters = { search: '', tipo: '', estado: '' };
    let subtipoIdToDelete = null;
    ...
    const dom = {
        ...
        modal: {
            ...
            form: document.getElementById('subtipo-form'),
            close: document.getElementById('close-modal-btn'),
            cancel: document.getElementById('cancel-btn'),
            inputs: {
                id: document.getElementById('subtipo-id'),
                tipo: document.getElementById('tipo-select'),
                nombre: document.getElementById('nombre-input'),
                descripcion: document.getElementById('descripcion-input'),
                estado: document.getElementById('estado-select')
            }
        },
        deleteModal: {
            self: document.getElementById('delete-modal'),
    ...
    // Core Functions
    function filterSubtipos() {
        let filtered = subtiposData;
        
        if (currentFilters.tipo) filtered = filtered.filter(s => s.id_tipo_incidencia == currentFilters.tipo);
        if (currentFilters.estado) filtered = filtered.filter(s => s.estado === currentFilters.estado);
        ...
        filteredSubtipos = filtered;
        renderSubtipos(filteredSubtipos);
        updateSearchInfo();
    }

    function updateSearchInfo() {
        if (currentFilters.search || currentFilters.tipo || currentFilters.estado) {
            const count = filteredSubtipos.length;
            ...
            if (currentFilters.tipo) {
                const tipo = tiposData.find(t => t.id_tipo_incidencia == currentFilters.tipo);
                if (tipo) filters.push(`tipo: ${tipo.nombre}`);
            }
    ...
    function renderSubtipos(subtipos) {
        ...
        if (subtipos.length > 0) {
            ...
            subtipos.forEach(s => {
                ...
                row.innerHTML = `
                    <td class="px-6 py-4 text-xs font-mono font-bold text-gray-400 group-hover:text-teal-600">#${s.id_subtipo_incidencia}</td>
                    <td class="px-6 py-4">
                        <div class="text-sm font-bold text-gray-900">${escapeHtml(s.nombre)}</div>
                    </td>
                    <td class="px-6 py-4">
                        <span class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-blue-50 text-blue-700 border border-blue-100">
                            ${escapeHtml(s.tipo_nombre || 'N/A')}
                        </span>
                    </td>
                    <td class="px-6 py-4">
                        <div class="text-sm text-gray-500 truncate max-w-xs">${escapeHtml(s.descripcion || '-')}</div>
                    </td>
                    <td class="px-6 py-4">
                        <span class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium ${
                            ...
                            <span class="w-1.5 h-1.5 rounded-full mr-1.5 ${s.estado === 'activo' ? 'bg-emerald-500' : 'bg-gray-400'}"></span>
                    ...
                    <td class="px-6 py-4 text-right">
                        <div class="flex items-center justify-end gap-2">
                            <button class="edit-btn p-2 text-teal-600 hover:bg-teal-50 rounded-lg transition-colors" 
                                data-id="${s.id_subtipo_incidencia}" 
                                title="Editar">
    ...
    async function loadData() {
        ...
        try {
            ...
            const subtiposResult = await subtiposRes.json();
            const tiposResult = await tiposRes.json();

            subtiposData = subtiposResult.records || [];
            tiposData = tiposResult.records || [];

            // Populate Selects
            const tipoOptions = tiposData.map(t => `<option value="${t.id_tipo_incidencia}">${escapeHtml(t.nombre)}</option>`).join('');
            dom.filters.tipo.innerHTML = '<option value="">Todos los tipos</option>' + tipoOptions;
            dom.modal.inputs.tipo.innerHTML = '<option value="">Seleccione un tipo...</option>' + tipoOptions;
    ...
    dom.filters.tipo.addEventListener('change', (e) => {
    ...
    dom.filters.reset.addEventListener('click', () => {
        dom.search.value = '';
        dom.filters.tipo.value = '';
        dom.filters.estado.value = '';
        currentFilters = { search: '', tipo: '', estado: '' };
        dom.clearSearch.classList.add('hidden');
        filterSubtipos();
    });
    ...
    document.getElementById('add-btn').addEventListener('click', () => {
        ...
        dom.modal.title.textContent = 'Nuevo Subtipo';
        dom.modal.self.classList.remove('hidden');
    });

    [dom.modal.close, dom.modal.cancel].forEach(btn => 
    ...
    dom.modal.form.addEventListener('submit', async (e) => {
        ...
        const formData = {
            nombre: dom.modal.inputs.nombre.value.trim(),
            descripcion: dom.modal.inputs.descripcion.value.trim(),
            id_tipo_incidencia: parseInt(dom.modal.inputs.tipo.value),
            estado: dom.modal.inputs.estado.value
        };

        if (id) formData.id_subtipo_incidencia = parseInt(id);
        ...
        try {
            ...
            if (result.success) {
                showToast(id ? 'Subtipo actualizado' : 'Subtipo creado', 'success');
    ...
    dom.tableBody.addEventListener('click', async (e) => {
        ...
        if (btn.classList.contains('edit-btn')) {
            ...
            try {
                ...
                if (result.subtipo) {
                    const s = result.subtipo;
                    dom.modal.inputs.id.value = s.id_subtipo_incidencia;
                    dom.modal.inputs.tipo.value = s.id_tipo_incidencia;
                    dom.modal.inputs.nombre.value = s.nombre;
                    dom.modal.inputs.descripcion.value = s.descripcion || '';
                    dom.modal.inputs.estado.value = s.estado;
                    dom.modal.title.textContent = 'Editar Subtipo';
                    dom.modal.self.classList.remove('hidden');
                }
            } catch (error) {
        ...
        if (btn.classList.contains('delete-btn')) {
            subtipoIdToDelete = btn.dataset.id;
    ...
    dom.deleteModal.confirm.addEventListener('click', async () => {
        ...
        try {
            const res = await fetch(API_URL, {
                method: 'DELETE',
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify({ id_subtipo_incidencia: parseInt(subtipoIdToDelete) })
            });
            const result = await res.json();
