...
<div class="container mx-auto px-4 py-8 max-w-7xl">
    ...
    <div class="flex flex-col lg:flex-row justify-between items-start lg:items-center mb-8 gap-4">
        <div>
            <h1 class="text-2xl font-bold text-gray-900">Roles de Usuario</h1>
            <p class="text-sm text-gray-500 mt-1">Define los niveles de acceso y permisos del sistema</p>
        </div>
        ...
        <div class="flex flex-col sm:flex-row gap-3 w-full lg:w-auto">
            ...
            <button id="add-rol-btn" class="bg-teal-600 hover:bg-teal-700 text-white font-medium py-2.5 px-5 rounded-xl shadow-lg shadow-teal-500/30 transition-all duration-300 flex items-center justify-center whitespace-nowrap transform hover:-translate-y-0.5">
    ...
    <div id="loading" class="hidden py-12 flex flex-col items-center justify-center text-teal-600">
        ...
        <p class="text-sm font-medium text-gray-500">Cargando roles...</p>
    </div>

    <!-- Table -->
    <div id="roles-table-container" class="bg-white shadow-sm border border-gray-100 rounded-2xl overflow-hidden">
        <div class="overflow-x-auto">
            <table class="w-full text-left border-collapse">
                ...
                <tbody id="roles-table-body" class="divide-y divide-gray-100">
    ...
    <div id="no-data-message" class="hidden bg-white shadow-sm border border-gray-100 rounded-2xl p-12 text-center">
        ...
        <h3 class="text-lg font-bold text-gray-900 mb-1">No hay roles registrados</h3>
        <p class="text-gray-500 mb-6">Comienza definiendo los roles de usuario del sistema.</p>
        <button onclick="document.getElementById('add-rol-btn').click()" class="text-teal-600 font-medium hover:text-teal-700">
            Crear primer rol &rarr;
        </button>
    </div>
    ...
    <div id="no-search-results" class="hidden bg-white shadow-sm border border-gray-100 rounded-2xl p-12 text-center">
        ...
        <p class="text-gray-500 mb-6">No encontramos roles que coincidan con tu búsqueda.</p>
...
<!-- Modal -->
<div id="rol-modal" class="fixed inset-0 z-50 hidden overflow-y-auto" aria-labelledby="modal-title" role="dialog" aria-modal="true">
    <div class="flex items-end justify-center min-h-screen pt-4 px-4 pb-20 text-center sm:block sm:p-0">
        ...
        <div class="inline-block align-bottom bg-white rounded-2xl text-left overflow-hidden shadow-xl transform transition-all sm:my-8 sm:align-middle sm:max-w-md sm:w-full">
            <div class="bg-white px-6 py-4 border-b border-gray-100 flex items-center justify-between">
                <h3 class="text-lg font-bold text-gray-900" id="modal-title">Nuevo Rol</h3>
            ...
            <form id="rol-form" class="px-6 py-6">
                ...
                <div class="mb-6">
                    ...
                    <input type="text" id="rol-nombre" required maxlength="20" pattern="[a-zA-Z0-9_]+"
                    ...
                    <p class="mt-2 text-xs text-gray-500">Solo letras, números y guiones bajos (máx. 20 caracteres)</p>
                </div>
            </form>
            ...
            <div class="bg-gray-50 px-6 py-4 flex flex-col-reverse sm:flex-row sm:justify-end gap-3 border-t border-gray-100">
                ...
                <button type="submit" id="save-btn" form="rol-form" class="w-full sm:w-auto px-6 py-2.5 bg-teal-600 text-white font-medium rounded-xl hover:bg-teal-700 shadow-lg shadow-teal-500/30 transition-all flex items-center justify-center">
                    <span id="save-btn-text">Guardar Rol</span>
...
<div id="delete-confirm-modal" class="fixed inset-0 z-50 hidden overflow-y-auto" aria-labelledby="modal-title" role="dialog" aria-modal="true">
    <div class="flex items-end justify-center min-h-screen pt-4 px-4 pb-20 text-center sm:block sm:p-0">
        ...
        <div class="inline-block align-bottom bg-white rounded-2xl text-left overflow-hidden shadow-xl transform transition-all sm:my-8 sm:align-middle sm:max-w-sm sm:w-full p-6 text-center">
            ...
            <h3 class="text-lg font-bold text-gray-900 mb-2">¿Eliminar rol?</h3>
            <p class="text-sm text-gray-500 mb-6">Esta acción no se puede deshacer. Asegúrate de que no haya usuarios activos con este rol.</p>
...
<script>
    ...
    const dom = {
        ...
        tableContainer: document.getElementById('roles-table-container'),
        ...
        modal: {
            self: document.getElementById('rol-modal'),
            title: document.getElementById('modal-title'),
            form: document.getElementById('rol-form'),
            ...
            inputs: {
                id: document.getElementById('rol-id'),
                nombre: document.getElementById('rol-nombre')
            }
        },
        deleteModal: {
    ...
    function filterRoles() {
        if (!currentSearchTerm.trim()) {
            ...
            filteredRoles = allRoles.filter(r => r.nombre_rol.toLowerCase().includes(term));
        }
    ...
    function renderRoles(roles) {
        ...
        if (roles.length > 0) {
            ...
            roles.forEach(r => {
                ...
                row.innerHTML = `
                    <td class="px-6 py-4 text-xs font-mono font-bold text-gray-400 group-hover:text-teal-600">#${r.id_rol}</td>
                    <td class="px-6 py-4">
                        <div class="text-sm font-bold text-gray-900 capitalize">${escapeHtml(r.nombre_rol)}</div>
                    </td>
                    <td class="px-6 py-4">
    ...
    // Modal Actions
    document.getElementById('add-rol-btn').addEventListener('click', () => {
        dom.modal.form.reset();
        dom.modal.inputs.id.value = '';
        dom.modal.title.textContent = 'Nuevo Rol';
    ...
    dom.modal.form.addEventListener('submit', async (e) => {
        ...
        const nombre = dom.modal.inputs.nombre.value.trim();
        
        if (!/^[a-zA-Z0-9_]+$/.test(nombre)) {
            showToast('Formato inválido: solo letras, números y _', 'error');
            return;
        }

        const formData = { nombre_rol: nombre };
        if (id) formData.id_rol = parseInt(id);
        ...
        try {
            ...
            if (res.ok) {
                showToast(result.message || 'Rol guardado', 'success');
            ...
            dom.modal.saveText.textContent = 'Guardar Rol';
    ...
    dom.tableBody.addEventListener('click', (e) => {
        ...
        if (btn.classList.contains('edit-btn')) {
            ...
            dom.modal.title.textContent = 'Editar Rol';
            dom.modal.self.classList.remove('hidden');
        }
        
        if (btn.classList.contains('delete-btn')) {
            rolIdToDelete = btn.dataset.id;
    ...
    dom.deleteModal.confirm.addEventListener('click', async () => {
        ...
        try {
            const res = await fetch(API_URL, {
                method: 'DELETE',
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify({ id_rol: parseInt(rolIdToDelete) })
            });
            const result = await res.json();
