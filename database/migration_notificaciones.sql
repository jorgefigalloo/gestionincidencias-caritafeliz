...
-- Fecha: 2025-12-22
-- Descripción: Agrega sistema de notificaciones por email
--              y workflow mejorado con confirmación de usuario
-- ============================================
...
-- Agregar campo email y preferencias de notificación
...
-- Actualizar emails de usuarios existentes (OPCIONAL - ajustar según tus datos)
...
-- Agregar campos de confirmación de usuario
...
-- Insertar configuración por defecto (DEBES ACTUALIZAR CON TUS CREDENCIALES)
...
-- ============================================
-- PASO 6: Crear TRIGGERS para automatización
-- ============================================

-- Trigger: Registrar cambio de estado automáticamente
DELIMITER $$

CREATE TRIGGER trg_historial_estado_update
...
BEGIN
    ...
    IF OLD.estado != NEW.estado THEN
        ...
        VALUES 
        (NEW.id_incidencia, OLD.estado, NEW.estado, NEW.id_usuario_tecnico, NEW.respuesta_solucion);
    END IF;
END$$
...
1. ACTUALIZAR EMAILS DE USUARIOS:
   - Edita los emails de los usuarios existentes en la tabla 'usuarios'
   - Puedes hacerlo manualmente o con UPDATE statements

2. CONFIGURAR SMTP:
   - Edita la tabla 'configuracion_email' con tus credenciales reales
   - Para Gmail, necesitas una "App Password" (no tu contraseña normal)
   - Activa "Acceso de aplicaciones menos seguras" o usa OAuth2

3. PERMISOS:
   - Verifica que el usuario de MySQL tenga permisos para crear triggers

4. TESTING:
   - Prueba el envío de notificaciones con datos de prueba
   - Verifica que los triggers funcionen correctamente

5. SIGUIENTE PASO:
   - Implementar la clase EmailNotifier.php
   - Modificar los controladores para usar el nuevo sistema
*/
